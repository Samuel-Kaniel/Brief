...
#### Remaining App Store steps

OTA updates are not a substitute for the first native binary. The store
target is the **iOS App Store**. `app.json` already has
`ios.bundleIdentifier` `com.samuelkaniel.brief`, version `1.0.0`,
`ios.buildNumber` `"1"`, and `ITSAppUsesNonExemptEncryption` /
`usesNonExemptEncryption` set to `false` (HTTPS only). `android.package` uses
the same id so the Expo config stays consistent; Play submission is out of
scope. Privacy-policy and support URLs are empty placeholders in
`expo.extra` until real pages are hosted.

Apple Developer membership, the App Store Connect record, TestFlight, hosted
privacy and support URLs, the first iOS production build, the ASC API key,
screenshots, age rating, and App Privacy labels are in
...
iOS credentials build locally:

```bash
npx eas-cli@latest build --platform ios --profile production
```

Submit to TestFlight with
...
The first iOS credentials pass still has to happen locally (see the App Store
steps above): native signing keys aren't generated by the workflow file
itself. On your machine, once:

```bash
npx eas-cli@latest build --platform ios --profile production
```

That interactive run stores the iOS distribution certificate on Expo's
servers so later iOS workflow builds can reuse it. The workflow file also
has an Android job; Play submission is out of scope for this release. Do not
commit keystores, `.p8`/`.p12` files, or other signing secrets — they stay
in EAS, not in this repo.
